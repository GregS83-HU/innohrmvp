...
const ROUTE_PATH = '../../src/app/api/update-next-step/route';
...
function req(candidat_id?: unknown, step_id: unknown = 3) {
  return new Request('http://localhost/api/update-next-step', { method: 'POST', body: JSON.stringify({ candidat_id, step_id }) });
}

describe('POST /api/update-next-step', () => {
  ...
  it('rejects with 401 when there is no session (previously: zero auth of any kind, service-role client bypassed RLS entirely)', async () => {
  ...
  it("rejects with 403 when the candidate is only linked to a different company's position (cross-tenant attempt)", async () => {
  ...
  it("updates the step when the candidate is linked to a position in the caller's own company", async () => {
    const { POST, wasUpdateCalled } = await loadRoute();
    const res = await POST(req(1));
