...
const RequestLeaveModalManual: React.FC<Props> = ({
  ...
  const handleFileChange = (selectedFile: File | null) => {
    ...
    if (selectedFile.size > MAX_SIZE) {
      setError(t('requestLeaveModal.errors.fileTooLarge'));
      setCertificateFile(null);
    } else {
  ...
  const validateCertificateData = (): boolean => {
    const errors: Record<string, string> = {};

    if (!certificateData.employee_name.trim()) {
      errors.employee_name = t('requestLeaveModal.validation.employeeNameRequired');
    }

    if (!certificateData.sickness_start_date) {
      errors.sickness_start_date = t('requestLeaveModal.validation.startDateRequired');
    }

    if (!certificateData.sickness_end_date) {
      errors.sickness_end_date = t('requestLeaveModal.validation.endDateRequired');
    }

    if (certificateData.sickness_start_date && certificateData.sickness_end_date) {
      if (certificateData.sickness_end_date < certificateData.sickness_start_date) {
        errors.sickness_end_date = t('requestLeaveModal.validation.endDateBeforeStart');
      }
    }

    if (!certificateFile) {
      errors.file = t('requestLeaveModal.validation.fileRequired');
    }
  ...
  const handleSaveCertificate = async () => {
    ...
    try {
      ...
      formData.append('employee_name', certificateData.employee_name);
      formData.append('absenceDateStart', certificateData.sickness_start_date);
      formData.append('absenceDateEnd', certificateData.sickness_end_date);
      formData.append('comment', certificateData.comment || '');
      ...
      // Sync dates with leave request form
      setRequestForm(prev => ({
        ...prev,
        start_date: certificateData.sickness_start_date,
        end_date: certificateData.sickness_end_date,
        reason: certificateData.comment || prev.reason
      }));
  ...
  const handleSubmitWithNotification = async (e: React.FormEvent) => {
    ...
    if (isSickLeave && !certificateId) {
      setError(t('requestLeaveModal.errors.confirmBeforeSubmit'));
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmitWithNotification} className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">
                      {t('requestLeaveModal.certificate.uploadFile')}
                    </label>
                    ...
                    } ${validationErrors.file ? 'border-red-300' : ''}`}>
                      <input
                        type="file"
                        accept=".pdf,image/*"
                    ...
                    {validationErrors.file && (
                      <p className="text-xs text-red-600 mt-1">{validationErrors.file}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCertificateData(prev => ({ ...prev, employee_name: e.target.value }))}
                    ...
                    {validationErrors.employee_name && (
                      <p className="text-xs text-red-600 mt-1">{validationErrors.employee_name}</p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        type="date"
                        value={certificateData.sickness_start_date}
                        onChange={(e) => {
                          const newStart = e.target.value;
                          setCertificateData(prev => ({
                            ...prev,
                            sickness_start_date: newStart,
                            // Clear end date if it's before the new start date
                            sickness_end_date: prev.sickness_end_date && prev.sickness_end_date < newStart 
                              ? '' 
                              : prev.sickness_end_date
                          }));
                        }}
                        className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                          validationErrors.sickness_start_date ? 'border-red-300' : 'border-gray-300'
                        }`}
                      />
                      {validationErrors.sickness_start_date && (
                        <p className="text-xs text-red-600 mt-1">{validationErrors.sickness_start_date}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={certificateData.sickness_end_date}
                        onChange={(e) => setCertificateData(prev => ({ ...prev, sickness_end_date: e.target.value }))}
                        min={certificateData.sickness_start_date}
                        className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                          validationErrors.sickness_end_date ? 'border-red-300' : 'border-gray-300'
                        }`}
                      />
                      {validationErrors.sickness_end_date && (
                        <p className="text-xs text-red-600 mt-1">{validationErrors.sickness_end_date}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      value={certificateData.comment}
                      onChange={(e) => setCertificateData(prev => ({ ...prev, comment: e.target.value }))}
          ...
          {/* Leave Request Date Fields - Only show if NOT sick leave */}
          {!isSickLeave && (
            <>
              ...
              {/* Reason - Only show if NOT sick leave */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
