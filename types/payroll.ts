...
export interface PayrollExportData {
    ...
    // NEW: Detailed breakdowns (arrays of allowances and deductions)
...
// Employee allowance (bonus, benefits, etc.)
export interface EmployeeAllowance {
    ...
    is_recurring: boolean; // If true, applies every month
    effective_month?: number; // 1-12
...
// Employee deduction (advance, loans, etc.)
export interface EmployeeDeduction {
    ...
    currency: string;
    total_amount?: number; // Total if installments
    remaining_amount?: number;
...
// Compensation summary for a period
export interface CompensationSummary {
    ...
    non_taxable_allowances: number;
    total_deductions: number;
    gross_total: number; // Base + allowances
    net_before_tax: number; // Gross - deductions (before income tax)
}

// Extended payroll export data with compensation
export interface PayrollExportDataWithCompensation extends PayrollExportData {
total_allowances: number;
taxable_allowances: number;
non_taxable_allowances: number;
total_deductions: number;
...
export interface UpdateAllowanceRequest extends Partial<CreateAllowanceRequest> {
    id: string;
}
...
export interface UpdateDeductionRequest extends Partial<CreateDeductionRequest> {
    id: string;
    is_active?: boolean;
    is_completed?: boolean;
}

// Helper function to get allowance type label
export function getAllowanceTypeLabel(type: AllowanceType): string {
...
// Helper function to get deduction type label
export function getDeductionTypeLabel(type: DeductionType): string {
    const labels: Record<DeductionType, string> = {
        advance_on_salary: 'Advance on Salary',
...
// Helper function to get tax treatment label
export function getTaxTreatmentLabel(treatment: TaxTreatment): string {
...
// Helper function to calculate net salary with compensation
export function calculateNetWithCompensation(
    baseSalary: number,
    allowances: number,
    deductions: number,
    ...
    const grossTotal = baseSalary + allowances;
    const netBeforeTax = grossTotal - deductions;
    const taxableIncome = baseSalary + taxableAllowances - familyTaxAllowance;

    const personalIncomeTax = taxableIncome * (HUNGARIAN_PAYROLL_CONSTANTS.PERSONAL_INCOME_TAX_RATE / 100);
    const socialContribution = (baseSalary + taxableAllowances) * (HUNGARIAN_PAYROLL_CONSTANTS.EMPLOYEE_SOCIAL_CONTRIBUTION / 100);

    const netSalary = netBeforeTax - personalIncomeTax - socialContribution;
...
// Hungarian-specific allowance configurations
export const HUNGARIAN_ALLOWANCE_CONFIG: Record<AllowanceType, {
    defaultTaxTreatment: TaxTreatment;
    taxFreeLimit?: number;
    description: string;
}> = {
    bonus: {
        defaultTaxTreatment: 'fully_taxable',
        description: 'Performance bonus, year-end bonus, etc.',
    },
    remboursement: {
        defaultTaxTreatment: 'non_taxable',
        description: 'Travel, business expense reimbursement',
    },
    cafeteria: {
        defaultTaxTreatment: 'tax_free_under_limit',
        taxFreeLimit: 450000, // HUF per year (example - verify current limit)
        description: 'SZÉP card, cafeteria benefits',
    },
    sport: {
        defaultTaxTreatment: 'tax_free_under_limit',
        taxFreeLimit: 150000, // HUF per year (example - verify current limit)
        description: 'Sport and recreation benefits',
    },
    cadeau: {
        defaultTaxTreatment: 'tax_free_under_limit',
        taxFreeLimit: 50000, // HUF per year (example - verify current limit)
        description: 'Gifts for employees',
    },
    other: {
        defaultTaxTreatment: 'fully_taxable',
...
// Format currency with allowances
