...
- Self-serve signup now exists on this app's own pricing page: a new company can create an account and start recruiting immediately, with no manual step. It's deliberately scoped rather than full-platform — payroll, time & attendance, absences, performance, the wellbeing chatbot, and (temporarily, for compliance reasons) medical certificate uploads stay locked behind a manual, per-company onboarding-call toggle regardless of plan, and that manual step has no scheduling or capacity tooling behind it yet.
