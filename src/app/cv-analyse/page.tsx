...
export default function CVAnalysePage() {
  ...
  //recupération de la job description
  const searchParams = useSearchParams()
  const jobDescriptionFromURL = searchParams.get('description') || ''
  const positionName = searchParams.get('position') || ''
  const [jobDescription, setJobDescription] = useState(jobDescriptionFromURL)
...
// Si la réponse est vide ou non JSON, le parse va planter
      if (!res.ok) {
        const text = await res.text() // lis le texte brut pour debug
  ...
  return (
    <main className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-center mb-6">📄 Analyse de CV avec ChatGPT pour la position </h1>
