...
export default function PositionsList({ initialPositions = [], companySlug }: Props) {
  ...
  useEffect(() => {
    ...
    fetchPositions()
  }, [companySlug, isLoggedIn, userId, initialPositions.length, t])
  ...
  const handleClose = useCallback(async (positionId: number) => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        setSnackbarMessage(`${t('positions.closeError')}: ${data.error || t('positions.unknownError')}`)
        setLoadingClose(null)
        return
      }
      setSnackbarMessage(t('positions.closedSuccess'))
      setPositions((prev) => prev.filter((p) => p.id !== positionId))
    } catch (e) {
      setSnackbarMessage(`${t('positions.closeError')}: ${(e as Error).message}`)
    }
    setLoadingClose(null)
  }, [t])
  ...
  const getPublicLink = useCallback((position: Position) => {
    if (!companySlug) return null
    const url = new URL(`/jobs/${companySlug}/cv-analyse?id=${position.id}`, window.location.origin)
    return url.toString()
  }, [companySlug])
  ...
  const handleCopyLink = useCallback(async (position: Position) => {
    ...
    try {
      ...
      setSnackbarMessage(t('positions.linkCopied'))
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-lg p-8 text-center max-w-sm w-full">
          ...
          <p className="text-red-600">{t('positions.error')}: {error}</p>
        </div>
      </div>
    )
  }
        ...
        <div className="text-center mb-6 sm:mb-8">
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-4 sm:mb-6">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">
              {t('positions.availablePositions')}
            </h1>
            <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-200 text-blue-700 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-sm sm:text-base">
              ...
              <span>{t('positions.positionsAvailable')}</span>
            </div>
          </div>
        ...
        {filteredPositions.length === 0 && (
          <div className="text-center">
            <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8">
              ...
              <h2 className="text-lg sm:text-xl font-semibold text-gray-600 mb-2">{t('positions.noPositions')}</h2>
              <p className="text-gray-500 text-sm sm:text-base">{t('positions.checkLater')}</p>
            </div>
          </div>
        )}
