...
export default function TicketDetailPage() {
  ...
  const fetchCurrentUser = useCallback(async (): Promise<UserData | null> => {
    try {
      ...
      if (!companyUserData) {
        ...
        setError(t('ticketDetail.errors.noCompanyAssociation'));
        return null;
      }
  ...
  const fetchTicket = useCallback(async (): Promise<TicketData | null> => {
    try {
      ...
      if (ticketError || !data) {
        setError(t('ticketDetail.errors.ticketNotFound'));
        return null;
      }
      ...
      const errorMessage = err instanceof Error ? err.message : t('ticketDetail.errors.loadTicket');
      setError(errorMessage);
      return null;
    }
  }, [params.ticketId, t]);
  ...
  const sendMessage = async (): Promise<void> => {
    ...
    try {
      ...
      const messageData = {
        ...
        sender_name: senderName || t('ticketDetail.common.unknownUser'),
      ...
      await supabase.from('notifications').insert({
        type: 'ticket_message',
        title: t('ticketDetail.notifications.newMessage'),
        message: t('ticketDetail.notifications.messageSent', { user: currentUser.user_firstname || t('ticketDetail.common.user') }),
        ticket_id: params.ticketId,
        sender_id: currentUser.auth_id
      });
  ...
  const updateTicketStatus = async (
    ...
    if (!isHrinnoAdmin || !ticket || !currentUser) return;
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        title: t('ticketDetail.notifications.statusUpdated'),
        message: t('ticketDetail.notifications.statusChanged', { 
          title: ticket.title, 
          status: t(`ticketDetail.status.${newStatus}`)
        }),
        ticket_id: ticket.id,
        sender_id: currentUser.auth_id
      });
      ...
      const errorMessage = err instanceof Error ? err.message : t('ticketDetail.errors.updateStatus');
      setError(errorMessage);
    } finally {
  ...
  const downloadAttachment = async (attachment: AttachmentData): Promise<void> => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : t('ticketDetail.errors.downloadFile');
      setError(errorMessage);
    }
  };
  ...
  const formatDate = (dateString: string): string => {
    ...
    if (diffInMinutes < 1) return t('ticketDetail.time.justNow');
    if (diffInMinutes < 60) return t('ticketDetail.time.minutesAgo', { count: Math.floor(diffInMinutes) });
    if (diffInMinutes < 1440) return t('ticketDetail.time.hoursAgo', { count: Math.floor(diffInMinutes / 60) });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-lg">
          ...
          <p className="text-gray-600 text-center">{t('ticketDetail.loading')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-lg text-center max-w-md w-full">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">
            {t('ticketDetail.errorState.title')}
          </h2>
          ...
          >
            {t('ticketDetail.errorState.goBack')}
          </button>
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-6">
          ...
          >
            ...
            {t('ticketDetail.header.backToTickets')}
          </button>
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{t('ticketDetail.header.created')} {formatDate(ticket.created_at)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                {isHrinnoAdmin && (
                  ...
                  >
                    ...
                    {t('ticketDetail.header.updateStatus')}
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 h-[600px] flex flex-col">
              ...
              <div className="p-4 border-b border-gray-100">
                <h3 className="font-semibold text-gray-900">{t('ticketDetail.messages.title')}</h3>
                <p className="text-sm text-gray-500">{t('ticketDetail.messages.count', { count: messages.length })}</p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.length === 0 ? (
                  <div className="text-center text-gray-500 py-8">
                    ...
                    <p>{t('ticketDetail.messages.empty')}</p>
                  </div>
                ) : (
              ...
              <div className="p-4 border-t border-gray-100">
                <div className="flex gap-2">
                  <textarea
                    ...
                    placeholder={t('ticketDetail.messages.placeholder')}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 flex flex-col h-[600px]">
              <h3 className="font-semibold text-gray-900 mb-4">{t('ticketDetail.attachments.title')}</h3>
              <div className="flex-1 overflow-y-auto space-y-3">
                {attachments.length === 0 ? (
                  <p className="text-gray-500 text-sm text-center mt-12">{t('ticketDetail.attachments.empty')}</p>
                ) : (
      ...
      {showStatusModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-80">
            <h3 className="font-semibold text-gray-900 mb-4">{t('ticketDetail.statusModal.title')}</h3>
                ...
                >
                  {updatingStatus ? (
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {t('ticketDetail.statusModal.updating')}
                    </div>
                  ) : (
                    t(`ticketDetail.status.${status}`).toUpperCase()
                  )}
                </button>
              ))}
            </div>
            ...
            >
              {t('ticketDetail.statusModal.cancel')}
            </button>
          </div>
        </div>
      )}
